...
// --- Guideline schema for ordered rules/instructions ---
...
// --- Prize schema for structured prize information ---
...
const arvantisSchema = new mongoose.Schema(
    {
        ...
        // --- changed: support multiple posters (array) instead of single poster ---
        posters: {
            type: [mediaSchema],
            default: [],
        },
...
// keep backwards-compatible virtual "poster" that returns first poster if present
arvantisSchema.virtual('poster').get(function () {
    if (Array.isArray(this.posters) && this.posters.length > 0) return this.posters[0];
    return null;
});
